...
source(Utils.getFilename("transmissions.lua", g_currentModDirectory))
...
function keyboardSteerMogli:ksmExternalSetOverwriteBrakeForce( value )
	if value then 
		self:ksmSetState( "ksmBrakeForce", KSMGlobals.brakeForceFactor )
	else 
		self:ksmSetState( "ksmBrakeForce", 1 )
	end 
end 

function keyboardSteerMogli:ksmExternalSetHideHud( value )
	if value then 
		self:ksmSetState( "ksmDrawHud", false )
	else 
		self:ksmSetState( "ksmDrawHud", true )
	end 
end 

function keyboardSteerMogli:ksmExternalGetHudPosition()
	if not self.ksmDrawHud then 
		return 0, 0, 0, 0
	end 
	...
	if KSMGlobals.snapAngleHudX >= 0 then 
		local x = KSMGlobals.snapAngleHudX
		local y = KSMGlobals.snapAngleHudY
...
function keyboardSteerMogli:onLoad(savegame)
	...
	--********************************************************************************************
	-- functions for others mods 
	self.ksmExternalSetShuttleCtrl         = keyboardSteerMogli.ksmExternalSetShuttleCtrl
	self.ksmExternalSetMovingDirection     = keyboardSteerMogli.ksmExternalSetMovingDirection
	self.ksmExternalSetOverwriteBrakeForce = keyboardSteerMogli.ksmExternalSetOverwriteBrakeForce
	self.ksmExternalSetHideHud             = keyboardSteerMogli.ksmExternalSetHideHud
	self.ksmExternalGetHudPosition         = keyboardSteerMogli.ksmExternalGetHudPosition
	-- functions for others mods 
	--********************************************************************************************
	...
	keyboardSteerMogli.registerState( self, "ksmExternalDir",  0 )
	...
	keyboardSteerMogli.registerState( self, "ksmDrawHud" ,     KSMGlobals.drawHud )
	...
	keyboardSteerMogli.registerState( self, "ksmShifterIndex", 0 )
	keyboardSteerMogli.registerState( self, "ksmShifterLH",    true )
...
keyboardSteerMogli.transmissions = {}
keyboardSteerMogli.transmissions[1] = keyboardSteerIVT:new()
keyboardSteerMogli.transmissions[2] = keyboardSteer4x4:new()
keyboardSteerMogli.transmissions[3] = keyboardSteer4PS:new()
keyboardSteerMogli.transmissions[4] = keyboardSteer2x6:new()
keyboardSteerMogli.transmissions[5] = keyboardSteerFPS:new()
keyboardSteerMogli.transmissions[6] = keyboardSteer6PS:new()


function keyboardSteerMogli:actionCallback(actionName, keyStatus)
	...
	if     actionName == "ksmGearUp"
			or actionName == "ksmGearDown"
			or actionName == "ksmRangeUp" 
			or actionName == "ksmRangeDown"
			or actionName == "ksmShifter1"
			or actionName == "ksmShifter2"
			or actionName == "ksmShifter3"
			or actionName == "ksmShifter4"
			...
			or actionName == "ksmShifter6"
			or actionName == "ksmShifter7"
			or actionName == "ksmShifterLH" then 
		for i,handler in pairs(keyboardSteerMogli.transmissions) do 
			if self.ksmTransmission == i then 
	...
	elseif actionName == "ksmINCHING" then 
		self:ksmSetState( "ksmInchingIsOn", keyStatus >= 0.5 )
	elseif actionName == "ksmNO_ARB" then 
		self:ksmSetState( "ksmNoAutoRotBack", keyStatus >= 0.5 )
...
function keyboardSteerMogli:onLeaveVehicle()
	...
	self.ksmPrevRotCursorKey = nil
	self:ksmSetState( "ksmSnapIsOn", false )
	self:ksmSetState( "ksmShifterIndex", 0 )
...
function keyboardSteerMogli:onUpdate(dt)
  ...
  if     self.spec_enterable         == nil
	...
	elseif self.ksmReverseDriveSample ~= nil then 
		...
		self.ksmReverseDriveSample               = nil
	end 
	...
	elseif self.ksmMaxForwardSpeed ~= nil then 
		...
		self.ksmMaxForwardSpeed  = nil
		self.ksmMaxBackwardSpeed = nil
	end 
	...
	if self:getIsActive() and self.isServer then
		if     self.ksmExternalDir > 0 then 
			self.ksmMovingDir = 1
		elseif self.ksmExternalDir < 0 then 
			self.ksmMovingDir = -1
		elseif self.ksmShuttleCtrl then 
...
--		and self.steeringEnabled 
		...
		elseif  g_gameSettings:getValue("isHeadTrackingEnabled") 
			...
			if self.ksmReverseIsOn or self.ksmCamBack ~= nil then			
				...
				if self.ksmReverseIsOn and not ( self.ksmCamFwd ) then 
					targetBack = true 
				end 
...
function keyboardSteerMogli:onDraw()
	if self:getIsVehicleControlledByPlayer() and not g_gui:getIsGuiVisible() then
		...
		if self.ksmShuttleCtrl or self.ksmTransmission ~= nil and self.ksmTransmission >= 2 then  
			setTextAlignment( RenderText.ALIGN_CENTER ) 
			setTextVerticalAlignment( RenderText.VERTICAL_ALIGN_MIDDLE )
			setTextColor(1, 1, 1, 1) 
		...
		if self.ksmDrawHud then 
			...
			if self.ksmTransmission >= 2 then 
				local transmission = keyboardSteerMogli.transmissions[self.ksmTransmission]
				local gear = transmission:getRatioIndex( self.ksmGear, self.ksmRange )		
				local maxSpeed = 3.6 * transmission:getGearRatio( gear ) * self.ksmMaxSpeed
				local text = transmission:getGearText( self.ksmGear, self.ksmRange )	
				local l2   = l
				
				if     self.ksmShifterIndex ==7 then 
					if self.ksmShifterLH then 
						text = "R+ ("..text..")"
					else 
						text = "R- ("..text..")" 
					end 
					l2 = l * 0.8
				elseif self.ksmShifterIndex > 0 then 
					if self.ksmShifterLH then 
						text = tostring(self.ksmShifterIndex).."+ ("..text..")"
					else 
						text = tostring(self.ksmShifterIndex).."- ("..text..")" 
					end 				
					l2 = l * 0.8
...
function keyboardSteerMogli:onReadStream(streamId, connection)
  ...
  self.ksmDrawHud       = streamReadBool(streamId) 
	self.ksmExternalDir   = streamReadInt16(streamId)     
...
function keyboardSteerMogli:onWriteStream(streamId, connection)
	...
	streamWriteBool(streamId, self.ksmDrawHud )     
	streamWriteInt16(streamId,self.ksmExternalDir )     
...
function keyboardSteerMogli:ksmUpdateWheelsPhysics( superFunc, dt, currentSpeed, acceleration, doHandbrake, stopAndGoBraking )
	...
	if self:getIsVehicleControlledByPlayer() then 		
		if self.ksmShuttleCtrl then 
			...
			if      self.spec_motorized.motor.ksmAutoStop
					and acceleration < 0.1 then 
				doHandbrake  = true 
			end			
		end 			
			
		if self.spec_drivable.cruiseControl.state == 0 and self.ksmLimitThrottle ~= nil and self.ksmInchingIsOn ~= nil and math.abs( acceleration ) > 0.01 then 
			local limitThrottleRatio     = 0.75
...
-- handbrake
	if self.vehicle.ksmNeutral then 
		...
		if self.vehicle.ksmShuttleCtrl and self.vehicle.ksmOldAcc ~= nil then 
	...
	local transmission = keyboardSteerMogli.transmissions[self.vehicle.ksmTransmission]
	...
	-- 4x4 / 4x4 PS / 2x6 / FPS 
		...
		local gear      = transmission:getRatioIndex( self.vehicle.ksmGear, self.vehicle.ksmRange )		
		...
		self.vehicle.ksmDebugR = string.format("g: %2d, r: %5.3f, s: %5.1f, m: %4.0f, w: %4.0f, c: %4.0f",
		...
		if curAcc < 0.1 and curBrake < 0.1 and self.ksmAutoDownTimer < 3000 then  
			self.ksmAutoDownTimer = 3000 
		end 
		if self.gearChangeTimer > 0 and self.ksmAutoDownTimer < 1000 then 
			self.ksmAutoDownTimer = 1000 
		end 
		if curAcc < 0.1 and self.ksmAutoUpTimer < 1000 then 
			self.ksmAutoUpTimer = 1000 
		end 
		if self.ksmClutchTimer > 0 and self.ksmAutoUpTimer < 500 then 
			self.ksmAutoUpTimer = 500 
		...
		if curAcc < 0.1 and self.ksmAutoLowTimer < 2000 then 
		...
		elseif  self.vehicle.ksmAutoShift 
			...
			if self.vehicle.ksmShifterIndex <= 0 then 
				newGear = self.vehicle.ksmLaunchGear
			end 
		elseif self.vehicle.ksmAutoShift and self.gearChangeTimer <= 0 and not self.vehicle.ksmNeutral and self.vehicle.ksmShifterIndex <= 0 then
			...
			if #gearlist > 0 then 
				...
				self.vehicle.ksmDebugA = string.format( "%3.0f%%; %3.0f%%; %4.0f..%4.0f; %4.0f -> %4.0f; %d -> %d; %5.0f -> %5.0f",
																								curAcc*100,self.ksmLoad*100,autoMinRpm,autoMaxRpm,wheelRpm, rr,gear,newGear,d1,d )
			end 
		end 
		
		if gear ~= newGear then 
			if self.vehicle.ksmDebugA ~= nil then 
				keyboardSteerMogli.debugPrint( g_currentMission.time.."; "..self.vehicle.ksmDebugA )
			end 
			
			local g, r = transmission:getBestGearRangeFromIndex( self.vehicle.ksmGear, self.vehicle.ksmRange, newGear )
			self.vehicle:ksmSetState( "ksmGear",  g )
			self.vehicle:ksmSetState( "ksmRange", r )
...
function keyboardSteerMogli:ksmShowSettingsUI()
	...
	self.ksmUI.ksmTransmission = { "off" }
	for _,transmission in pairs(keyboardSteerMogli.transmissions) do 
		table.insert( self.ksmUI.ksmTransmission, transmission:getName() )
	end 
	...
	if transmission ~= nli then 
		for i,g in pairs(transmission:getRatioIndexList( i ) ) do
			self.ksmUI.ksmLaunchGear[i] = string.format( "%3.0f km/h", transmission:getGearRatio( g )*3.6*self.ksmMaxSpeed )
		end 
	end 
