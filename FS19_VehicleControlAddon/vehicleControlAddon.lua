...
function vehicleControlAddon.getClutch( xmlFile, xmlProp, xmlAttr )
	local text = getXMLString( xmlFile, xmlProp..'#'..xmlAttr )
...
function vehicleControlAddon.setClutch( xmlFile, xmlProp, xmlAttr, value )
...
function vehicleControlAddon:onLoad(savegame)
	...
	self.vcaResetMotorProperties= vehicleControlAddon.vcaResetMotorProperties
	...
	vehicleControlAddon.registerState( self, "vcaRotAccTime",   vehicleControlAddon.getRotAccTime( self ) )
	...
	vehicleControlAddon.registerState( self, "vcaOwnRangeTime"  , 1000  , function( self, ... ) vehicleControlAddon.vcaOnSetOwn( self, "vcaOwnRangeTime"  , ... ) end )
...
function vehicleControlAddon:onUpdate(dt, isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
	...
	elseif self.vcaIsBlocked and self.vcaIsEntered then 
	-- do not change while a dialog is active
	...
	if self.vcaKSIsOn and self.vcaIsEntered then	
		...
		elseif math.abs( self.spec_drivable.axisForward ) > 0.01 then 
			...
			elseif self:vcaGetAutoShift() then 
				s = math.max( s, 3.6 * ( 0.9 * motor.minRpm + 0.1 * motor.maxRpm ) * self.vcaLaunchSpeed / motor.maxRpm )
			end 
...
--******************************************************************************************************************************************
	if self.isServer and self:getIsActive() and self.vcaIsLoaded and self.firstTimeRun then  
		...
		self.vcaMaxWheelSlip = self.vcaMaxWheelSlip + 0.04 * ( ws - self.vcaMaxWheelSlip )
...
function vehicleControlAddon:getRotAccTime()
	if      self.spec_motorized ~= nil 
			and self.spec_motorized.motor ~= nil 
			and self.spec_motorized.motor.minRpm ~= nil 
			and self.spec_motorized.motor.maxRpm ~= nil 
			and self.spec_motorized.motor.motorRotationAccelerationLimit ~= nil
			and self.spec_motorized.motor.motorRotationAccelerationLimit > 0
			then 
		local motor = self.spec_motorized.motor
		return math.floor( (motor.maxRpm - motor.minRpm)*vehicleControlAddon.factorpi30 * 1000 / motor.motorRotationAccelerationLimit + 0.5 )
	end 
	return 2
end 
...
function vehicleControlAddon:vcaUpdateGear( superFunc, acceleratorPedal, dt )
	...
	local lastFakeRpm         = Utils.getNoNil( self.vcaFakeRpm,  math.max( self.motorRotSpeed * vehicleControlAddon.factor30pi, self.minRpm )) 
	...
	local maxRotAcc = self.motorRotationAccelerationLimit 
	if self.vehicle.vcaRotAccTime ~= nil and self.vehicle.vcaRotAccTime > 0 then 
		maxRotAcc = (self.maxRpm - self.minRpm)*vehicleControlAddon.factorpi30 * 1000 / self.vehicle.vcaRotAccTime
	end 
	-- idle acceleration w/o load, gear, ...
	local maxRpmAccDt = maxRotAcc * vehicleControlAddon.factor30pi * 0.0015 * dt
	...
	if self.vehicle.vcaRatedRpm ~= nil and self.vehicle.vcaRatedRpm < self.maxRpm then 
		rpmRange = self.vehicle.vcaRatedRpm - self.minRpm 
	end 
...
--end 
	...
	local maxExpAggrUseMax = false 
--if self.vehicle.vcaMaxWheelSlip == nil or self.vehicle.vcaMaxWheelSlip > 0.2 then 
--	maxExpAggrUseMax = true 
--end 
	...
	if self.vcaLastNValues == nil then 
		...
		for i=2,100 do 
			table.insert( self.vcaLastNValues, self.vcaLastNValues[1] )
		end 
	elseif self.vcaMaxWheelAcc ~= nil and self.vcaMaxMotorAcc ~= nil then 
	  ...
	  local m = self.vcaLastNValues[iMax].m + 0.001 * dt * self.vcaMaxMotorAcc
		...
		for i=1,iMax-1 do 
			if self.vcaLastNValues[i].t + dt < 1000 then 
				local wi = self.vcaLastNValues[i].w + 0.001 * ( self.vcaLastNValues[i].t + dt ) * self.vcaMaxWheelAcc 
				local mi = self.vcaLastNValues[i].m + 0.001 * ( self.vcaLastNValues[i].t + dt ) * self.vcaMaxMotorAcc
					
				if maxExpAggrUseMax then 
					w = math.max( w, wi )
					m = math.max( m, mi )
					n = 1 
				else 
		...
		if n > 1 then 
			...
			expectedWheelSpeed        = w
			expectedMotorSpeed        = m
		end 
	end 
	...
	clampedWheelSpeed = math.max( wheelSpeed, math.min( speedMS, expectedWheelSpeed ) )
		
	if self.vcaDirTimer ~= nil or self.gearChangeTimer > 0 or self.vcaWheelAccS == nil then 
		self.vcaWheelAccS = 0
		self.vcaMotorAccS = 0
	else 
		self.vcaWheelAccS = self.vcaWheelAccS + 0.04 * ( self.vcaWheelAcc - self.vcaWheelAccS )
		self.vcaMotorAccS = self.vcaMotorAccS + 0.04 * ( self.vcaMotorAcc - self.vcaMotorAccS )
	end 
	...
	if     curBrake > 0.1
			...
			or self.vcaMotorAccS    > maxRotAcc   
			then   
	...
	if     self.vehicle.vcaHandthrottle == nil 
		...
		idleRpm = math.max( idleRpm, math.min( motorPtoRpm, self.minRpm + a * rpmRange ) )
	...
	elseif self.vehicle.vcaHandthrottle > 0 then 
		...
		if self.vehicle:vcaGetNoIVT() and self.vehicle:vcaGetAutoShift() then 
			idleRpm        = math.min( idleRpm, self.minRpm + 0.9 * rpmRange )
		end 
	elseif not self.vcaIsEnteredMP then 
	...
	local fakeRpmS    = vehicleControlAddon.mbClamp( fakeRpm, lastFakeRpm - maxRpmAccDt, lastFakeRpm + maxRpmAccDt )	
	...
	elseif self.vcaFakeTimer ~= nil then 
		if math.abs( lastFakeRpm - motorRpm ) < 2 then 
			...
			if     self.vcaFakeTimer <= 0  then 
				self.vcaFakeTimer = nil 
			elseif self.vcaFakeTimer < 100 then 
				self.vcaFakeRpm = 0.01 * self.vcaFakeTimer * fakeRpmS + ( 1 - 0.01 * self.vcaFakeTimer ) * motorRpm 
			else 
				self.vcaFakeRpm = vehicleControlAddon.mbClamp( motorRpm, lastFakeRpm - maxRpmAccDt, lastFakeRpm + maxRpmAccDt )	
			end 
		end 
	end 
	if self.vcaFakeTimer ~= nil then
		idleRpm = math.max( idleRpm, lastFakeRpm - maxRpmAccDt )
	end 
	...
	if autoNeutral or curAcc < 1e-3 then 
		...
		self.vcaUsedPowerRatio  = self.motorExternalTorque * self.motorRotSpeed
	else 
		local t = self.motorAppliedTorque 
		
		mat = math.max(self.motorAvailableTorque, 0.0001)
		if motorRpm > self.minRpm + rpmRange and self.vcaRatedTorque ~= nil then  
			mat = self.vcaRatedTorque
		end 
	...
	if self.gearChangeTimer > 0 or self.vehicle.vcaClutchDisp > 0 then 
		self.vcaUsedPowerRatioS = math.max( self.vcaUsedPowerRatioS, lastUsedPowerRatio )
	end 
	
	if speed > lastRealSpeedLimit - 1 and curAcc > self.vcaUsedPowerRatioS then  
		curAcc = math.max( math.min( curAcc, 0.2 ), self.vcaUsedPowerRatioS )
	end 
	...
	if self.vcaWheelSlipF == nil then
		self.vcaWheelSlipF = self.vcaWheelSlip
	else 
		self.vcaWheelSlipF = self.vcaWheelSlipF + 0.04 * ( self.vcaWheelSlip - self.vcaWheelSlipF )
	end 
...
--****************************************************************************************	
		...
		if      self.vehicle.vcaMaxWheelSlip ~= nil 
				and self.vehicle.vcaMaxWheelSlip > 0.1  
				and self.vehicle.vcaAntiSlip then 
			self.speedLimit = math.min( self.speedLimit, math.max( 0, speed ) + 1 )
		end 
...
--****************************************************************************************	
		...
		if motorRpm < self.minRpm and self.vcaFakeTimer == nil then  
			self.vcaFakeRpm   = fakeRpmS	
		...
		if self.vehicle.spec_combine ~= nil then 
			minReducedRpm = math.min( math.max( newMinRpm, self.minRpm + 0.6 * rpmRange ), newMaxRpm )
	...
	--minReducedRpm = math.min( minReducedRpm + 0.2 * rpmRange, newMaxRpm ) 
		...
		if self.vehicle.spec_combine == nil and self.vehicle.vcaLimitThrottle ~= nil and self.vehicle.vcaInchingIsOn ~= nil then 
			...
			if self.vehicle.spec_drivable.cruiseControl.state == 0 then
				...
				if self.vehicle.vcaInchingIsOn == limitThrottleIfPressed then 
					newMaxRpm = math.max( minReducedRpm, math.min( newMaxRpm, self.minRpm + rpmRange * limitThrottleRatio ) )
				end 
			end 
			...
			local basePR  = self:getTorqueCurveValue( baseRpm ) * baseRpm * vehicleControlAddon.factorpi30
			if     basePR <= 0 then 
				basePR = 0 
			elseif basePR >= self.vcaPeakPower then 
				basePR = 1
			else 
				basePR = basePR / self.vcaPeakPower
			end 
		...
		if self.vcaFakeRpm ~= nil then 
			self.vcaFakeRpm = math.min( math.max( self.vcaFakeRpm, self.vcaMinRpm ), lastFakeRpm + maxRpmAccDt )
		end 
...
--****************************************************************************************	
		...
		if     self.vcaClutchTimer == nil 
			...
			autoNeutral      = true 
			self.vcaAutoStop = true	
			lastAutoStopNoRpm   = g_currentMission.time  
		...
		elseif  curBrake > 0.1
				...
				and ( motorRpm < 0.9 * self.minRpm or speed < 2 )
		...
		if lastGearRatio ~= nil and lastGearRatio > 0 and self.vehicle.vcaMaxSpeed > 0 then  
			prevRatio = self.maxRpm / ( lastGearRatio * self.vehicle.vcaMaxSpeed * vehicleControlAddon.factor30pi ) 
		end 
	
		local wheelRpm  = clampedWheelSpeed * self.maxRpm / maxSpeed 
			
		self.vehicle.vcaDebugR = debugFormat("g: %2d, r: %5.3f, s: %5.1f, m: %4.0f, w: %4.0f",
																	gear, ratio, maxSpeed*3.6,
																	self.lastRealMotorRpm, wheelRpm )
		...
		local rMin, rMax = self.minRpm + 0.1 * rpmRange, self.minRpm + 0.9 * rpmRange
		if     self.vcaWheelAccS >  0.05 and self.vcaAutoDownTimer < 250 and motorRpm > rMin then  
			self.vcaAutoDownTimer = 250
		elseif self.vcaWheelAccS < -0.05 and self.vcaAutoUpTimer   < 250 and motorRpm < rMax then  
...
--****************************************************************************************	
			...
			wheelRpm  = clampedWheelSpeed * self.maxRpm / maxSpeed 
...
--****************************************************************************************	
			...
			if self.vehicle:vcaGetAutoShift() then
				...
				if setLaunchGear then
					...
					self.vcaSetLaunchGear = true 
				elseif self.gearChangeTimer <= 0 and not self.vehicle:vcaGetNeutral() then --and lastGearChangeTime == nil then
					local m1 = self.minRpm + 0.05 * rpmRange
					local m4 = self.minRpm + 0.95 * rpmRange 
					local autoMinRpm, autoMaxRpm
					if     self.vehicle.vcaHandthrottle > 0 then 
					-- handthrottle
						autoMaxRpm = vehicleControlAddon.mbClamp( idleRpm + 0.05 * rpmRange, m1, m4 )
						autoMinRpm = vehicleControlAddon.mbClamp( idleRpm - 0.05 * rpmRange, m1, m4 )
					elseif motorPtoRpm < self.minRpm then 
					-- no PTO
						m4 = math.min( self.vcaMaxPowerRpmH, m4 )
						if curBrake > 0.5 and self.vcaBrakeTimer ~= nil and self.vcaBrakeTimer > 0 then 
							m1 = math.min( math.max( m1, self.minRpm + math.min( 0.6, self.vcaBrakeTimer * 0.0005 ) * rpmRange ), m4 )
						elseif hasActiveWorkArea then  
							m1 = math.min( math.max( m1, self.minRpm + 0.4 * rpmRange ), m4 )
						end 
						if  curBrake < 0.1 and curAcc > 0.1 and curAcc < 0.8 and self.gearChangeTimer <= 0 then 
							m4 = math.max( m1, math.min( m4, self.minRpm + curAcc * rpmRange * 0.975 ) )
						end 
						m1 = m1 + autoShiftLoad * ( vehicleControlAddon.mbClamp( self.vcaMaxPowerRpmL, m1, m4 ) - m1 )
						autoMinRpm = m1
						autoMaxRpm = m4 
					elseif -1 < self.vehicle.vcaHandthrottle and self.vehicle.vcaHandthrottle < -0.8 then 
					-- 90% PTO 
						autoMaxRpm = vehicleControlAddon.mbClamp( motorPtoRpm + 0.05 * rpmRange, m1, m4 )
						autoMinRpm = vehicleControlAddon.mbClamp( idleRpm     - 0.05 * rpmRange, m1, m4 )
					elseif self.vehicle.vcaHandthrottle < 0 then 
					-- PTO ECO
						autoMaxRpm = vehicleControlAddon.mbClamp( idleRpm     + 0.05 * rpmRange, m1, m4 )
						...
						autoMaxRpm = vehicleControlAddon.mbClamp( motorPtoRpm + 0.05 * rpmRange, m1, m4 )
						autoMinRpm = vehicleControlAddon.mbClamp( motorPtoRpm - 0.05 * rpmRange, m1, m4 )
					end 
					
					if     wheelRpm > self.minRpm + 0.99 * rpmRange then 
						self.vcaAutoUpTimer = 0
					...
					local rpmLo      = math.min( wheelRpm, speedMS * self.maxRpm / maxSpeed )
					local rpmHi      = math.max( wheelRpm, speedMS * self.maxRpm / maxSpeed )

					local searchUp   = ( rpmHi > autoMinRpm and self.vcaAutoUpTimer   <= 0 and self.vcaNoShiftTimer <= 0 )
					local searchDown = ( rpmLo < autoMaxRpm and self.vcaAutoDownTimer <= 0 and self.vcaNoShiftTimer <= 0 )
					...
					if wheelRpm < self.minRpm or wheelRpm > self.minRpm + rpmRange then 
						if self.vcaAutoShiftEmergency == nil then 
							self.vcaAutoShiftEmergency = g_currentMission.time 
						end 
					else
						self.vcaAutoShiftEmergency = nil 
					end 
					...
					elseif wheelRpm <= 1 then 
						...
						local d = 0
						if self.vcaAutoShiftEmergency ~= nil and self.vcaAutoShiftEmergency + 2000 < g_currentMission.time then 
							d = math.huge 
						end 
						...
						local p0 = autoShiftLoad * self.vcaRatedPower
						local p1 = self:getTorqueCurveValue( wheelRpm ) * wheelRpm * vehicleControlAddon.factorpi30
						local p  = math.max( 0, p0 - p1 )
						...
						for _,i in pairs( gearlist ) do 
							...
							if      i ~= gear 
								...
								local rf = nil
								if nr > ratio and self.vehicle:vcaIsVehicleControlledByPlayer() and self.vcaGearUpRpmFactor ~= nil and self.vcaGearUpRpmFactor < 1 then 
									local g, r = transmission:getBestGearRangeFromIndex( self.vehicle.vcaGear, self.vehicle.vcaRange, i )
									local t = 0
									if g ~= self.vehicle.vcaGear  then t = math.max( t, transmission:getChangeTimeGears() ) end
									if r ~= self.vehicle.vcaRange then t = math.max( t, transmission:getChangeTimeRanges()) end
									if t > 100 then 
										rf = self.vcaGearUpRpmFactor
									end 
								end 
								
								local rpm = rpmLo * ratio / nr
								if rf ~= nil then rpm = rpm * rf end 
								local d2 = 0
								if rpm < autoMinRpm then 
									d2 = autoMinRpm - rpm
								end 
								
								local p2 = math.max( 0, p0 - self:getTorqueCurveValue( rpm ) * rpm * vehicleControlAddon.factorpi30 )
								...
								p2 = math.max( p2, p0 - self:getTorqueCurveValue( rpm ) * rpm * vehicleControlAddon.factorpi30 )
																
								if d2 < d or ( d2 == d and p2 < p ) or ( d2 == d and p2 == p and rpm < rr ) then 
									newGear = i 
									d  = d2
									p  = p2 
									rr = rpm
						...
						self.vehicle.vcaDebugA = debugFormat( "%3.0f%%; %3.0f%%; %4.0f..%4.0f;\n%4.0f -> %4.0f; %d -> %d; %5.0f -> %5.0f\n%4d; %4.1f; %s; %s",
...
--****************************************************************************************	
		...
		if not autoNeutral then 
			...
			if self.vcaLastRange ~= nil and self.vehicle.vcaRange ~= self.vcaLastRange then 
				...
				lastGearChangeSpeed = clampedWheelSpeed
			end 
		...
		if      self.gearChangeTimer > 100
				and curBrake             > 0.5
				and self.vcaBrakeTimer  ~= nil
				and self.vcaBrakeTimer   > 100
				and self.vehicle:vcaGetAutoShift()
				then
			self.gearChangeTimer = 100
		end 
		...
		elseif self.vcaGearIndex ~= nil and self.vcaGearIndex ~= gear then 
			if ratio > prevRatio then 
				...
				self.vcaAutoDownTimer = math.max( self.vcaAutoDownTimer, 4000 + self.gearChangeTimer )
				self.vcaNoShiftTimer  = VCAGlobals.minAuthShiftWait 
				if self.gearChangeTimer > 0 and wheelRpm > self.minRpm then 
				-- start calculation of drop of wheel RPM after gear shifting
					self.vcaShiftWheelRpm = wheelRpm 
				end 
			else                                    
				...
				if curBrake <= 0.1 then 
					...
					self.vcaNoShiftTimer = math.min( 100, VCAGlobals.minAuthShiftWait ) 
				end 
			end
		end 
		...
		if     autoNeutral or self.vcaDirTimer ~= nil or lastFwd ~= fwd  or not self.vehicle:vcaIsVehicleControlledByPlayer() then 
		-- ignore
		elseif self.vcaShiftWheelRpm ~= nil then 
		-- clear after shifting wheel RPM
			self.vcaShiftMaxWheelRpm = nil 
			self.vcaWorkRpmFactor2   = nil 
		elseif lastShiftWheelRpm == nil then  
		-- ignore 
		elseif self.vcaClutchTimer > 0 or self.vcaClutchClosedTime == nil then 
		-- remember wheel RPM before gear shifting 
			self.vcaShiftWheelRpm = lastShiftWheelRpm 
		elseif self.vcaShiftMaxWheelRpm == nil or self.vcaClutchClosedTime + 2000 > g_currentMission.time then 
		-- still remember wheel RPM before shifting and maximize wheel RPM after shifting 
			self.vcaShiftWheelRpm = lastShiftWheelRpm 
			if self.vcaShiftMaxWheelRpm == nil then 
				self.vcaShiftMaxWheelRpm = wheelRpm 
				self.vcaWorkRpmFactor2   = self.vcaGearUpRpmFactor
			else 
				self.vcaShiftMaxWheelRpm = math.max( self.vcaShiftMaxWheelRpm, wheelRpm )
			end 
			rpmFactor = self.vcaShiftMaxWheelRpm / lastShiftWheelRpm
			if self.vcaWorkRpmFactor2 == nil then 
				self.vcaGearUpRpmFactor = rpmFactor 
			else 
				self.vcaGearUpRpmFactor = self.vcaWorkRpmFactor2 + 0.1 * ( rpmFactor - self.vcaWorkRpmFactor2 )
			end 
		end 		
		...
		if lastGearRatio ~= nil and self.gearChangeTimer <= 0 and self.vcaClutchTimer <= 0 and lastGearChangeTime ~= nil then 
			...
			if lastGearRatio < curGearRatio then 
				if     curBrake >= 0.5 then 
					t = -1 
				elseif curBrake <= 0 then 
					t = 500 
				else 
					t = ( 0.5 - curBrake ) * 100
				end 
			...
			if t > 0 and g_currentMission.time - lastGearChangeTime < t then 
				...
				if lastGearRatio < curGearRatio then 
					...
					self.minGearRatio       = curGearRatio
					self.maxGearRatio       = f * curGearRatio + ( 1 - f ) * lastGearRatio
				end 
				...
				if lastGearChangeSpeed ~= nil and clampedWheelSpeed > 1.02 * lastGearChangeSpeed and newAcc > maxAcc then
					newAcc = maxAcc
				end 
			end 
		end 
...
--****************************************************************************************	
-- begin of clutch control => output: clutchFactor 
--****************************************************************************************	
		local clutchFactor    = 0
		local clutchOpenRpm   = 0
		local clutchCloseRpm  = self.minRpm
		local clutchCloseTime = 0
	
		if autoNeutral or self.gearChangeTimer > 0 or self.vehicle.vcaClutchPercent >= 1 then 
			self.vcaClutchTimer        = VCAGlobals.clutchTimer
			clutchFactor               = 1
			...
			elseif self.vehicle.vcaClutchMode == listOfClutchModes.turboOpen then 
				clutchRpmInc = 2
				clutchCloseTime = math.huge 
			elseif self.vehicle.vcaClutchMode == listOfClutchModes.turboManu then 
				clutchRpmInc = 0.4
			end
			
			clutchCloseRpm = math.max( self.minRpm + clutchRpmInc * rpmRange, idleRpm - 0.1 * rpmRange ) 
			...
			local autoOpen = ( ( curBrake > 0.1 ) or ( self.vehicle:vcaGetTurboClutch() and clutchDir < 0.1 ) ) and ( launchRatio == nil or ratio <= launchRatio * 1.1 )
			local clutchTimerMax = VCAGlobals.clutchTimer * 0.97
		
			if      self.vcaClutchTimer == nil or autoNeutral then 
				self.vcaClutchTimer   = VCAGlobals.clutchTimer
			elseif  lastAutoStopNoRpm  ~= nil
					and motorRpm            < fakeRpm 
					and motorRpm            < clutchCloseRpm 
					and motorRpm            < self.vcaMaxPowerRpmL
			...
			elseif self.vcaClutchTimer > 0 then 
			-- close clutch slowly
				self.vcaClutchTimer   = math.min( self.vcaClutchTimer - clutchDir * clutchDt, VCAGlobals.clutchTimer -1 )
				if self.vcaClutchTimer <= 0 and self.vehicle:vcaGetTurboClutch() and clutchDir < 0.1 then 
					self.vcaClutchTimer = 1 
				elseif self.vcaClutchTimer < 0 then 
					self.vcaClutchTimer = 0
				end 
			elseif wheelRpm < 0.85 * self.minRpm and autoOpen then 
			-- automatically open clutch completely
				self.vcaClutchTimer = math.max( self.vcaClutchTimer, clutchTimerMax )
			elseif wheelRpm < clutchOpenRpm and autoOpen then 
			-- automatically open clutch slowly
				self.vcaClutchTimer = math.max( self.vcaClutchTimer, math.min( self.vcaClutchTimer + clutchDt, VCAGlobals.clutchTimer - 1 ) )
			end

			if motorRpm < 0.7 * self.minRpm and not self.vcaAutoStop and self.vcaFakeRpm == nil then
			-- motor stall
			...
			if not ( self.vehicle:vcaGetAutoClutch() or self.vehicle:vcaGetTurboClutch() ) then 
			-- manual clutch w/o turbo clutch 
				self.vcaClutchTimer          = 0
				clutchFactor                 = self.vehicle.vcaClutchPercent
				self.vehicle.vcaClutchDisp   = self.vehicle.vcaClutchPercent
			elseif  self.vcaClutchTimer >  0
					and lastClutchClose     ~= nil 
					and wheelRpm            >= clutchCloseRpm 
					then 
			-- close clutch 
				self.vcaClutchClose = lastClutchClose - dt 				
				if self.vcaClutchClose <= 0 then 
					self.vcaClutchClose = nil 
					self.vcaClutchTimer = 0
					clutchFactor        = 0
				end 
			else 
			-- merge clutch pedal with automatic clutch timer 
				if     self.vehicle.vcaClutchPercent >= 1 then 
					self.vcaClutchTimer = VCAGlobals.clutchTimer
				elseif self.vehicle.vcaClutchPercent >  0 then 
					self.vcaClutchTimer = math.max( self.vcaClutchTimer, math.sqrt( self.vehicle.vcaClutchPercent ) * VCAGlobals.clutchTimer )
				end 
				if self.vehicle.vcaClutchMode == listOfClutchModes.turboOpen then
					self.vcaClutchTimer = math.max( 300, self.vcaClutchTimer )
				else 
					self.vcaClutchClose = clutchCloseTime 
				end 
				...
				clutchFactor = math.max( self.vcaClutchTimer / VCAGlobals.clutchTimer, 0 ) ^ 2
...
--****************************************************************************************	
-- end of clutch control
--****************************************************************************************	


--****************************************************************************************	
-- begin of min/max gear ratio and min/max RPM (needs clutchFactor and curGearRatio)
--****************************************************************************************	
		if clutchFactor >= 1 then 
		-- neutral or no gear
			if self.gearChangeTimer > 0 and not autoNeutral then 
				if self.vcaFakeRpm == nil or self.gearChangeTimer > 500 then  
					self.vcaFakeRpm = vehicleControlAddon.mbClamp( vehicleControlAddon.mbClamp( wheelRpm, self.minRpm, self.maxRpm ), 
																												 lastFakeRpm - maxRpmAccDt, lastFakeRpm + maxRpmAccDt )		
				elseif lastFakeRpm < wheelRpm then 
					self.vcaFakeRpm = math.max( lastFakeRpm - 0.33 * maxRpmAccDt, idleRpm )
				else 
					self.vcaFakeRpm = math.max( lastFakeRpm - maxRpmAccDt, idleRpm )
				end 
				self.vcaFakeTimer = 100 
			elseif self.vcaFakeRpm == nil then 
				self.vcaFakeRpm   = vehicleControlAddon.mbClamp( fakeRpm, lastFakeRpm - maxRpmAccDt, lastFakeRpm + maxRpmAccDt )			
				self.vcaFakeTimer = 100 
			end
			...
			if clampedWheelSpeed <= 1e-6 then 
				self.minGearRatio        = 1 
				self.maxGearRatio        = 1000000
			else 
				local t = self.vcaFakeRpm
				if     vehicleControlAddon.factorpi30 * t <= clampedWheelSpeed then 
					self.minGearRatio      = 1
				elseif vehicleControlAddon.factorpi30 * t >= 1000000 * clampedWheelSpeed then 
					self.minGearRatio      = 1000000
				else 
					self.minGearRatio      = vehicleControlAddon.factorpi30 * t / clampedWheelSpeed
				end 
				self.maxGearRatio        = self.minGearRatio
			end 
			self.vehicle.vcaClutchDisp = 1
			self.vcaClutchClosedTime   = g_currentMission.time 
		else
			self.vcaMinRpm = 0
			...
			if     clutchFactor > 0 then 
				...
				local function getRelativeRPM( r1, r0 ) 
					...
					return clutchFactor * r1 + ( 1 - clutchFactor ) *  r0 
				end 
				
				local function getRPM( r ) 
				-- allow to reach idle RPM at full throttle 
					return math.max( getRelativeRPM( r, w ), math.min( getRelativeRPM( self.maxRpm, w ), idleRpm ) )
				end 
				
				local function getACR( rpm )
					if self.vehicle:vcaGetAutoClutch() or self.vehicle:vcaGetTurboClutch() then 
						return math.min( rpm, math.max( clutchCloseRpm + 0.1 * rpmRange, wheelRpm, lastFakeRpm - maxRpmAccDt ))
					end 				
					return rpm 
				end 
				
				local function getGR( rpm )
					if rpm < 20 * curGearRatio * wheelSpeed * vehicleControlAddon.factor30pi then 
						return math.max( 0.05 * curGearRatio, rpm / ( wheelSpeed * vehicleControlAddon.factor30pi ) )
					end 
					return 20 * curGearRatio 
				end 
				...
				-- make sure the interval opens over time, i.e. if the maxRpmAccDt is too slow 
				self.vcaMinRpm = math.min( self.vcaMinRpm, clutchFactor * m )
				self.vcaMaxRpm = math.max( self.vcaMaxRpm, ( 1 - clutchFactor ) * m )
				
				self.vcaClutchClosedTime   = g_currentMission.time 
			end  

			self.speedLimit = math.min( self.speedLimit, math.max( vehicleControlAddon.maxRpmF1 * maxSpeed, speedMS * ( 1 - 0.001 * dt ) ) * 3.6 )
...
--****************************************************************************************	
-- end of min/max gear ratio and min/max RPM
--****************************************************************************************	
	...
	if curGearRatio ~= nil then 
		rpm = curGearRatio * clampedWheelSpeed * vehicleControlAddon.factor30pi
	end 
	...
	-- neutral
		lowROI = true 
	elseif  curAcc > 0.1 and self.vcaWheelAcc ~= nil and self.vcaWheelAcc > self.accelerationLimit then 
	-- acceleartion to high => increase inertia
		f = VCAGlobals.rotInertiaFactorMax
	elseif  ( self.vehicle.vcaClutchDisp > 0 or curGearRatio == nil )
			and self.gearChangeTimer <= 0
			and rpm < self.minRpm + 0.1 * rpmRange
			and self.vcaWheelAccS < 0.2
			and math.abs( self.vcaLastAccS ) > 0.5 then 
	-- problems to launch => reduce inertia
		lowROI = true 
	elseif  curGearRatio == nil then 
	-- IVT 
		if  self.vcaAutoStop or fwd ~= lastFwd or self.vcaDirTimer ~= nil or speed < 1 then  
			lowROI = true 
		else 
		-- rotInertiaFactorIVT	
			f = VCAGlobals.rotInertiaFactorIVT
		end 
	else--if  curGearRatio ~= nil then 
	-- high ROI if vehicle is in a very low gear 
		-- rotInertiaFactorInc	
		-- rotInertiaFactorMax	
		f =  math.min( VCAGlobals.rotInertiaFactorMax, math.max( VCAGlobals.rotInertiaFactor, curGearRatio * VCAGlobals.rotInertiaFactorInc ) )
	end 
	
	if lowROI then 
		f = VCAGlobals.rotInertiaFactorLow 
	end 
	if self.vehicle.vcaROIFactorS ~= nil then 
	-- smooth 
		f = vehicleControlAddon.mbClamp( f, self.vehicle.vcaROIFactorS - 0.001 * dt, self.vehicle.vcaROIFactorS + 0.001 * dt )
	end 
	...
	if      curAcc > 0.1
			...
			and speed > 2
			and not lowROI
			and motorRpm > self.minRpm + 0.1 * rpmRange 
			and self.vehicle.vcaDiffManual then 
		local f, m, b = self.vehicle:vcaGetDiffState()
		if f == 2 or m == 2 or b == 2 then 
	...
	if self.vcaDirTimer ~= nil or self.vcaMaxAccFactor == nil or ( self.vehicle.vcaClutchDisp > 0 and speed < 10 ) then   
		...
		local maxAccFactor    = math.max( 0.01, math.min( 2 * ( 1 - self.vcaUsedTorqueRatioF ), 1, math.abs( newAcc ) ) )
		...
		self.vcaMaxMotorAcc   = self.vcaMaxAccFactor * maxRotAcc

		local acc = 0
		local dt2 = math.max( 10, self.vehicle.vcaTickDt )
		
		acc = self.accelerationLimit
		...
		if lastAccSpeedLimit == nil or wheelSpeed >= 0.99*expectedWheelSpeed then 
			self.vcaAccSpeedLimit = math.min( self.vcaAccSpeedLimit, self.vcaMaxWheelSpeed )
			dbg = "x" 
		end
		if     speed > lastRealSpeedLimit then 
			self.vcaMaxWheelAccE = 0
		elseif self.vcaMaxWheelAccE ~= nil then 
		elseif speed >= 5 then 
			self.vcaMaxWheelAccE = 2 * self.accelerationLimit
		elseif speed <= 0 then 
			self.vcaMaxWheelAccE = self.accelerationLimit
		else 
			self.vcaMaxWheelAccE = ( 1 + 0.2 * speed ) * self.accelerationLimit
		end

		if      ( lastAccSpeedLimit == nil or self.motorRotSpeed >= 0.99*expectedMotorSpeed )
				and curGearRatio ~= nil 
				and self.vcaFakeTimer == nil 
				and self.gearChangeTimer <= 0 
				and self.vehicle.vcaClutchDisp <= 0 
				and self.vcaWheelSlipS < 0.1 then 
		...
		if self.vcaMaxMotorAccE == nil then 
			self.vcaMaxMotorAccE = maxRotAcc
		end 	
		
		if self.speedLimit > self.vcaAccSpeedLimit * 3.6 then 
			dbg = "!" .. dbg 
			self.speedLimit = self.vcaAccSpeedLimit * 3.6
		else 
			dbg = "." .. dbg 
		end 
...
function vehicleControlAddon:vcaGetTotalConsumedPtoTorque( superFunc, excludeVehicle, ... )
	...
	if VCAGlobals.clutchMinusTorqueT > 1 and self:vcaGetTurboClutch() then 
		f = f / VCAGlobals.clutchMinusTorqueT 
	end 
	if f <= 1.01 then 
		return superFunc( self, excludeVehicle, ... )
	end 
	
	if self.vcaMaxWheelSlip ~= nil and self.vcaMaxWheelSlip < 0.1 and VCAGlobals.clutchMinusTorqueA > 0 and motor.vcaWheelAccS ~= nil and motor.vcaWheelAccS > 0 then 
		f = math.min( f, math.max( VCAGlobals.clutchMinusTorqueM, motor.vcaWheelAccS * VCAGlobals.clutchMinusTorqueA ), 1000 )
	else 
		f = math.min( f, VCAGlobals.clutchMinusTorqueM )
	end 
	...
	if p > motor.motorAvailableTorque then 
		return superFunc( self, excludeVehicle, ... )
	end 
	...
	return motor.ptoMotorRpmRatio * ( motor.vcaClutchMinusTorque + p )
end 
...
function vehicleControlAddon:vcaResetMotorProperties()
	local motor = self:getMotor() 
	if motor ~= nil then 
		motor.vcaLastVehicleDamage = nil 
	end 
	self:updateMotorProperties()
end 
...
function vehicleControlAddon:vcaUpdateMotorProperties( superFunc, ... )
	...
	if motor ~= nil and self.vcaGearbox ~= nil then 
		...
		motor.vcaRealMinRpm = motor.minRpm 
		motor.vcaRealMaxRpm = motor.maxRpm 
		
		motor.minRpm = 0
		motor.maxRpm = vehicleControlAddon.maxRpmF1 * motor.vcaRealMaxRpm
	end 
	
	superFunc( self, ... )
	
	if reset then 
		motor.minRpm = motor.vcaRealMinRpm
		motor.maxRpm = motor.vcaRealMaxRpm
	end 
...
function vehicleControlAddon:vcaMotorGetRatedRpm()
	if self.vehicle == nil then 
		return 0.0001
	elseif self.vehicle.vcaRatedRpm ~= nil then 
		return self.vehicle.vcaRatedRpm  
	elseif self.vcaRealMaxRpm ~= nli then 
		return self.vcaRealMaxRpm 
	end 
	return self.maxRpm 
end 

function vehicleControlAddon:vcaMotorGetRatedTorque()
	local r = vehicleControlAddon.vcaMotorGetRatedRpm( self )
	
	if self.vcaRatedTorqueNoDamage == nil then 
		if self.vehicle.vcaRatedPower > 0 then
			self.vcaRatedTorqueNoDamage = self.vehicle.vcaRatedPower / ( r * vehicleControlAddon.factorpi30 )
		else
			self.vcaRatedTorqueNoDamage = self:getTorqueCurve():get( r )
		end 
	end 
	
	return self.vcaRatedTorqueNoDamage
end 
...
function vehicleControlAddon:vcaGetTorqueCurveValue( superFunc, rpm )
	...
	local rRated = vehicleControlAddon.vcaMotorGetRatedRpm( self )
	
	if not ( self.vehicle ~= nil and self.vehicle.vcaIsLoaded ) then 
	-- 
	elseif self.vehicle.vcaTorqueCurve ~= nil and self.vehicle.vcaTorqueCurve > 0 then  
		local tRated = vehicleControlAddon.vcaMotorGetRatedTorque( self )
	...
	elseif self.vehicle.vcaGearbox ~= nil 
		...
		local factor = 1
		if self.vehicle.vcaRatedPower > 0 then   
			factor = self.vehicle.vcaRatedPower / math.max( 0.01, self:getTorqueCurve():get( rRated ) * rRated * vehicleControlAddon.factorpi30 )
	...
	if self.vehicle ~= nil and self.vehicle.vcaIsLoaded and self.vehicle.vcaGearbox ~= nil then 
		if self.vcaMaxPowerRpmL == nil or self.vcaLastVehicleDamage == nil or self.vcaLastVehicleDamage ~= self.vehicle:getVehicleDamage() then  
			...
			if type( self.torqueCurve ) == "table" and type( self.torqueCurve.keyframes ) == "table" then 
				numKeyFrames = #self.torqueCurve.keyframes
			end 
			if numKeyFrames < 1 and self.vehicle.vcaTorqueCurve <= 0 then  
				self.vcaMaxPowerRpmL = self.minRpm 
				self.vcaMaxPowerRpmH = self.maxRpm 
				self.vcaPeakPower    = 0
				self.vcaRatedPower   = 0
				self.vcaRatedTorque  = 0
				self.vcaPeakTorque   = 0
				self.vcaPeakPowerRpm = self.maxRpm
			else
				local listL = { { r = 0, p = 0 } }
				local listH = { { r = 0, p = 0 } }

				self.vcaPeakPower    = 0
				self.vcaPeakPowerRpm = 0

				local tMax, vLast = 0, 0
				local tOver, vOver
				
				local c, f, r
				local r = Utils.getNoNil( self.vcaRealMaxRpm, self.vehicle.vcaRatedRpm )

				self.vcaRatedTorqueNoDamage = nil 
				vehicleControlAddon.vcaMotorGetRatedTorque( self )
				
				self.vcaRatedTorque = self:getTorqueCurveValue( r )
				self.vcaRatedPower  = self.vcaRatedTorque * r * vehicleControlAddon.factorpi30
				
				if     self.vehicle.vcaTorqueCurve == 1 then 
					c = vehicleControlAddon.torqueCurve1
					f = r
					self.vcaPeakTorque = self.vcaRatedTorqueNoDamage / c:get(1)
				elseif self.vehicle.vcaTorqueCurve == 2 then 
					c = vehicleControlAddon.torqueCurve2
					f = r
					self.vcaPeakTorque = self.vcaRatedTorqueNoDamage / c:get(1)
				...
				elseif self.vehicle.vcaTorqueCurve == 5 then 
					...
					c = self.torqueCurve
					f = 1
					self.vcaPeakTorque = c:getMaximum()
				end 
				...
				for i,v in ipairs(c.keyframes) do
					...
					if i == 1 and rpm > 0 then  
						table.insert(self.vcaRotationSpeeds, 0)
						table.insert(self.vcaTorqueValues, 0)
					end

					table.insert(self.vcaRotationSpeeds, rotSpeed)
					table.insert(self.vcaTorqueValues, torque)
					tMax  = rpm
					vLast = torque 
					if self.vehicle.vcaTorqueCurve <= 0 and #self.vcaRotationSpeeds == 3 and self.vcaTorqueValues[3] > self.vcaTorqueValues[2] and self.vcaRotationSpeeds[3] > self.vcaRotationSpeeds[2] then
						self.vcaTorqueValues[1] = math.max( 0, self.vcaTorqueValues[3] - self.vcaRotationSpeeds[3] * ( self.vcaTorqueValues[3] - self.vcaTorqueValues[2] ) / ( self.vcaRotationSpeeds[3] - self.vcaRotationSpeeds[2] ) ) 
					end 
					
					if self.vcaRealMaxRpm ~= nil and tMax > self.vcaRealMaxRpm then 
						if p > self.vcaPeakPower then 
							break 
				...
				for i,rotSpeed in ipairs(self.vcaRotationSpeeds) do
					local rpm    = rotSpeed / vehicleControlAddon.factorpi30
					local torque = self.vcaTorqueValues[i]
					
					local p = rotSpeed*torque
				
					if rpm >= self.vcaPeakPowerRpm then 
						table.insert( listL, { r = rpm, p = p } )
						break 
					elseif p > 0.97 * self.vcaPeakPower then 
						table.insert( listL, { r = rpm, p = p } )
					elseif #listL == 1 then  
						listL[1].r = rpm 
						listL[1].p = p 
				...
				-- last real torque value at vcaRealMaxRpm 
				if self.vcaRealMaxRpm ~= nil and tMax < self.vcaRealMaxRpm - 1 and tOver ~= nil then  			
					vLast = vLast + ( self.vcaRealMaxRpm - tMax ) * ( vOver - vLast ) / ( tOver - tMax )
					tMax  = self.vcaRealMaxRpm
					table.insert(self.vcaRotationSpeeds, tMax*vehicleControlAddon.factorpi30)
					table.insert(self.vcaTorqueValues, vLast )
				end 
				-- at a smooth ending
				...
				if numKeyFrames > 1 then 
					...
					local p = 0.97 * self.vcaPeakPower
				...
				if numKeyFrames > 1 then 
					local i = 2 
					local p = 0.99 * self.vcaPeakPower
				...
				if self.vcaRealMaxRpm ~= nil and self.vcaMaxPowerRpmH > r then 
			...
			for i=1,#self.vcaRotationSpeeds do	
				local r = self.vcaRotationSpeeds[i]*30/math.pi
				local t = self.vcaTorqueValues[i]*1000
				local p = 1.36 * self.vcaRotationSpeeds[i] * self.vcaTorqueValues[i]
		...
		--vehicleControlAddon.debugPrint(string.format("%5.0f .. %5.0f", self.vcaMaxPowerRpmL, self.vcaMaxPowerRpmH ))
			vehicleControlAddon.debugPrint(string.format("%5.0f .. %5.0f", self.vcaRealMinRpm, self.vcaRealMaxRpm ))
...
function vehicleControlAddon:vcaSetROIFactor( f ) 
	self.vcaROIFactorS = f 
	f = 0.01 * math.floor( f * 100 + 0.5 )
	if f ~= nil and math.abs( self.vcaROIFactor - f ) > 0.005 then 
		self.vcaROIFactor = f
		self:updateMotorProperties() 
	end 
end 
...
function vehicleControlAddon:vcaMotorGetRotInertia( superFunc ) 
	...
	if      type( r ) == "number"     
			...
			and self.vehicle.vcaIsLoaded  
			and self.vehicle.vcaGearbox   ~= nil 
			and self.vcaPeakTorque        ~= nil then 
		return self.vehicle.vcaROIFactor * self.vcaPeakTorque / 600
	end 
	return r
end 
...
function vehicleControlAddon:onSetTransmission( old, new, noEventSend )
	...
	if      old ~= nil 
			and vehicleControlAddonTransmissionBase.transmissionList[old] ~= nil
			and vehicleControlAddonTransmissionBase.transmissionList[new] ~= nil
			and vehicleControlAddonTransmissionBase.transmissionList[old].defaultClutch ~= nil  
			and vehicleControlAddonTransmissionBase.transmissionList[new].defaultClutch ~= nil  
			and vehicleControlAddonTransmissionBase.transmissionList[old].defaultClutch == self.vcaClutchMode then 
		self:vcaSetState( "vcaClutchMode", vehicleControlAddonTransmissionBase.transmissionList[new].defaultClutch, noEventSend )
	end 
	self:requestActionEventUpdate()
end 
...
function vehicleControlAddon:vcaShowSettingsUI()
	...
	self.vcaUI.vcaBrakeForce_V = { 0, 0.01, 0.02, 0.05, 0.10, 0.15, 0.2, 0.25, 0.4, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2 }
	...
	for i=0,5000,250 do 
		table.insert( self.vcaUI.vcaRotAccTime,   string.format( "%4d ms", i  ) )
	end 
...
function vehicleControlAddon:vcaUIGetvcaRotAccTime() 
	return vehicleControlAddon.mbClamp( math.floor( self.vcaRotAccTime / 250 + 0.5 ), 0, 20 )
end 
function vehicleControlAddon:vcaUISetvcaRotAccTime( value )
	self:vcaSetState( "vcaRotAccTime", value * 250 )
end 
