...
function vehicleControlAddon:vcaUpdateGear( superFunc, acceleratorPedal, dt )
	...
	elseif autoNeutral or self.gearChangeTimer > 0 then 
		...
		if curLoad > self.vcaLoad then 
			self.vcaLoad = self.vcaLoad + 0.05  * ( curLoad - self.vcaLoad ) 
		else 
			self.vcaLoad = self.vcaLoad + 0.015 * ( curLoad - self.vcaLoad ) 
		end 
	end 
	...
	--	newMinRpm = vehicleControlAddon.mbClamp( self.vehicle.lastSpeedReal * 1000 * self.maxRpm / ( self.vehicle.vcaMaxSpeed * self.vehicle.vcaGearRatioF ), self.minRpm, self.maxRpm )
	--end 
	--if self.vehicle.vcaGearRatioT > 0 then 
	--	newMaxRpm = vehicleControlAddon.mbClamp( self.vehicle.lastSpeedReal * 1000 * self.maxRpm / ( self.vehicle.vcaMaxSpeed * self.vehicle.vcaGearRatioT ), self.minRpm, self.maxRpm )
	--end 
	...
	--end 
		...
		elseif self.vehicle:vcaGetAutoShift() and self.gearChangeTimer <= 0 and not self.vehicle:vcaGetNeutral() and self.vehicle.vcaShifterIndex <= 0 then
			...
			if wheelRpm > m4 and self.vcaAutoUpTimer > 250 then 
			...
			if self.vcaClutchTimer <= 0 and wheelRpm < m1 and gear > launchGear and self.vcaAutoDownTimer > 250 then 
