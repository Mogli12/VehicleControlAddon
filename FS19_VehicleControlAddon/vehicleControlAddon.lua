...
--****************************************************************************************	
	...
	--end 
		...
		elseif self.vehicle:vcaGetAutoShift() and self.gearChangeTimer <= 0 and not self.vehicle:vcaGetNeutral() and self.vehicle.vcaShifterIndex <= 0 then
			...
			if #gearlist > 0 then 
				...
				self.vehicle.vcaDebugA = string.format( "%3.0f%%; %3.0f%%; %4.0f..%4.0f; %4.0f -> %4.0f; %d -> %d; %5.0f -> %5.0f",
																								curAcc*100,self.vcaLoad*100,autoMinRpm,autoMaxRpm,self.vcaLastRpmW, rr,gear,newGear,d1,d )
			else 
				self.vehicle.vcaDebugA = string.format( "%3.0f%%; %3.0f%%; %4.0f..%4.0f; %4.0f no gear",
																								curAcc*100,self.vcaLoad*100,autoMinRpm,autoMaxRpm,self.vcaLastRpmW)
			end 
		end
		...
		elseif motorRpm < 0.8 * self.minRpm and not self.vcaAutoStop and self.vcaFakeRpm == nil then 
			if curBrake > 0.1 or self.vehicle:vcaGetAutoShift() or self.vehicle.vcaTurboClutch then   
		...
		elseif wheelRpm < minRequiredRpm and ( curBrake > 0.1 or self.vehicle:vcaGetAutoShift() or self.vehicle.vcaTurboClutch ) then 
		...
		--end 
			...
			if clutchFactor > 0.5 and motorRpm < clutchCloseRpm then 
				self.vcaFakeRpm   = Utils.getNoNil( lastFakeRpm, clutchCloseRpm )
				self.vcaFakeTimer = 100
			end 
